...
def test_swap_by_newest_between_good_vendors():
    # Arrange
    item_a = Decor(condition=2, age=3)
    item_b = Electronics(condition=4)
    item_c = Decor(condition=4, age=3)
    tai = Vendor(
        inventory=[item_c, item_b, item_a],
        trait="Good"
    )

    item_d = Clothing(condition=2, age=2)
    item_e = Decor(condition=4)
    item_f = Clothing(condition=4, age=2)
    jesse = Vendor(
        inventory=[item_f, item_e, item_d],
        trait="Good"
    )
